...
void trace(vm_t *vm) {
  if (vm == NULL) { return; }
  stack_t *grey_objects = stack_new(8);
  if (grey_objects = NULL) { return NULL; }
  for (int i = 0; i < vm->objects->count; i++) {
    snek_object_t *obj = vm->objects->data[i];
    if (obj->is_marked == true) {
        stack_push(grey_objects, obj);
    }
  }
  while (grey_objects->count > 0) {
    snek_object_t *obj = stack_pop(grey_objects);
    trace_blacken_object(grey_objects, obj);
  }
  stack_free(grey_objects);
